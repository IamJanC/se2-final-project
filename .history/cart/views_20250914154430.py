...
def add_to_cart(request, product_id):
    ...
    if cart_item:
        ...
        if cart_item.quantity >= product.stock:
            messages.error(request, "TEST: You already have max stock.")
            print("DEBUG: message added")
            return redirect(request.POST.get('next') or 'products:product_detail', product_id=product.id)

        
        if new_quantity > product.stock:
            error_message = f"Only {product.stock} of {product.name} available."
            return render(request, "products/product_detail")
