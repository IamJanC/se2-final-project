...
function showOptions(key) {
  ...
  // Check if last bot message is the same prompt to avoid duplicates
  const lastBotMessage = [...chatLog.querySelectorAll('.bot.message-content')].pop();
  if (!lastBotMessage || lastBotMessage.innerText.trim() !== group.prompt.trim()) {
    const time = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
